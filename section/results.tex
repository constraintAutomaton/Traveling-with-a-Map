...
Figure~\ref{fig:compApproach} shows that the shape index approach performs better or comparably
\rv{be specific; what is \enquote{better}, what is \enquote{comparably}. Use exact words, and numbers where applicable.}
to the state-of-the-art Solid Pod network traversal algorithms, for all query templates except S4.
...
The shape index approach allows for answering queries from the S7 template,
\rv{Be specific and scientific here. It's not \enquote{answering}, it's obtaining results. The other approaches were \emph{able} to process the query, but not to produce the (non-zero) result~set within the timeout.}
which was not possible by the other approaches, due to large number of HTTP requests resulting in timeouts.
\rv{More specific, again: because a~large number of requests would be needed, and could not be completed within the chosen timeout.}
Queries can require as little as 13\% of the execution time (S1 being 7 times faster) of the type index.
\rv{Be more exact/precise. The \enquote{execution time of the type index} is not a thing.}
The queries that perform the best are those in which the number of HTTP requests decreased the most,
\rv{That we already knew before we started, what's new here?}
as can be inferred from the analysis of Table~\ref{tab:ratioUsefulResources}.
...
\rv{Wow we're claiming a Nobel prize for measuring here. We can't reliably measure sub-ms, and it's not meaningful to express in percentage of timeout.}
