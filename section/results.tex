...
\begin{figure*}
	...
	\caption{
		...
		The shape index performs better than both the type index and \ac{LDP} for data-model selective queries.
...
\begin{figure}
	\includesvg[width=\linewidth]{analysis/artefact/variation_approach/s1_violon_plots}
	\caption{
...
\begin{figure}
	\includesvg[width=\linewidth]{analysis/artefact/variation_approach/d7_violon_plots}
	\caption{
...
\begin{figure}
	\includesvg[width=\linewidth]{analysis/artefact/variation_approach/s4_violon_plots}
	\caption{
...
In the \hyperref[sec:appendix]{appendix} Figure~\ref{fig:ratioApproaches} presents the ratio of execution time between the type index approach and the shape index approach.
Table~\ref{tab:statSignificanceStateOfTheArt} presents the statistical significance by query templates.
The shape index approach allows for successful termination of S7 template queries, which other approaches cannot complete due to timeouts resulting from an excessive number of \ac{HTTP} requests.
...
The queries that perform the best are those in which the number of \ac{HTTP} requests decreased the most, as can be inferred from the analysis of Table~\ref{tab:ratioUsefulResources}.
This table presents the average percentage of query-relevant resources per query template, derived from the where-provenance~\cite{buneman2001and} of the query results and the number of \ac{HTTP} requests.
Queries with a high decrease of the number of \ac{HTTP} request are known to be data-model selective due to the query-shape subsumption algorithm.
...
From the analysis of the queries, those derived from templates D1, D4, S1, S3, and S7 (S3 is a template that cannot complete the execution before the timeout, while S7 can only be executed successfully with the shape index) are identified as data-model-selective and, as expected, exhibit a reduction in both the number of \ac{HTTP} requests and the query execution time.
...
\rtwo{The D6 and D7 templates are presented in Listings~\ref{lst:d6} and~\ref{lst:d7}.}
...
\rtwo{The S4 template is presented in Listing~\ref{lst:s4}.}
...
In contrast, the shape index approach always enforces the use of these properties, resulting in additional \ac{HTTP} requests and increased processing time.
...
The shape index must also be retrieved whenever traversal reaches a new subweb, which can stall the engine between two consecutive results and widen the gaps counted as waiting time, even when the total query execution time is reduced or remains unchanged.
...
\paragraph{The Shape Index Approach Can Improve the Diefficiency Metric}
...
The \textit{dief@lr} show a similar behavior but with lesser performance gain compared to \textit{dief@1s} (see Table~\ref{tab:dief} to compare the average values).
...
Entries mapped to an open shape are always considered relevant because the shape translates to a query fetching the whole \ac{KG}.
...
\begin{figure*}[htbp]
	...
	% General caption
	\caption{
		The data show two regimes in the relation between the number of \ac{HTTP} requests and the execution time.
		The correlation is more linear in the left figure than in the right figure.
	}
...
To study the relationship between the number of \ac{HTTP} request and the query execution time, the ratio of \ac{HTTP} request of the different approaches and the ratio of query execution time was
calculated in relation to the type index approach.
Figure~\ref{fig:http_req_exec_time_cor} presents our analysis.
The relationship between \ac{HTTP} request and query execution time can be divided into two regimes.
In the first regime (left figure), where the shape index approach reduces the number of \ac{HTTP} requests, we notice a positive linear correlation with a \ac{PCC} of 0.84 and a high statistical significance ($< 0.01$).
...
Above a ratio of approximately 0.85 of \ac{HTTP} requests, the shape index approach did not guarantee a reduction in query execution time.
...
However, the query-shape subsumption algorithm execution time is negligible for one execution and the state retention has a polynomial time complexity, thus it should not have a high impact on the execution time.
Another explanation is the number of \ac{HTTP} requests that are performed in parallel.
The \ac{LTQP} version of Comunica performs 10 \ac{HTTP} requests in parallel, thus, we would expect that with a low number and ratio of \ac{HTTP} requests, the performance would remain largely unchanged or slightly worse, which is what can be observed in the curve.

In the second regime (right figure), the shape index increases the number of \ac{HTTP} requests.
We notice a moderate positive linear correlation with a \ac{PCC} of 0.44.
The overall correlation between reducing \ac{HTTP} requests and query execution time is positively linear, with a moderate linear correlation with a \ac{PCC} of 0.56 and a high statistical significance.
...
In contrast, the second regime shows a much flatter slope of around 0.08, suggesting that the ratio of \ac{HTTP} requests has a weaker influence.
This disparity indicates that the explanation may be more complex than just a sample size issue.
The increased number of \ac{HTTP} requests can be attributed to queries executed using the D6, D7, and S4 templates, with the S4 template causing the largest increase.
Queries from the S4 template typically require only 1 or 2 \ac{HTTP} requests, so when running with 10 concurrent requests, the impact on performance is limited.
On the other hand, queries using the D6 and D7 templates, executed with the type index traversal algorithm, required 26, 23, and 129 \ac{HTTP} requests.
This significantly higher number of \ac{HTTP} requests explains why the increase in \ac{HTTP} requests had a greater impact on query execution time.

These results partially validate \textbf{H7}, as a linear relationship is observed when there are a sufficient number of \ac{HTTP} requests and a significant reduction ratio, taking concurrent requests into account.

\subsection{\rone{Summary of Findings}}

\rone{Across the evaluation, the shape index approach reduced both the number of retrieved sources and the query execution time for data-model selective queries while preserving result completeness, but it degraded performance for queries that do not exploit the structural properties of the network, such as those of template S4.
	The query-shape subsumption algorithm introduced negligible overhead.
	Reducing the amount of shape index information was not consistently detrimental: neither stricter constraints nor more complete indexes guaranteed better performance, yet the approach retained its gains even under partial coverage.
	Finally, the reduction in \ac{HTTP} requests correlated with the decrease in execution time only when a sufficient number of requests were involved.
	Table~\ref{tab:hypothesisSummary} summarizes, for each hypothesis, whether the evaluation supports it.}

\begin{table*}[htbp]
	\centering
	\ifreview\color{revone}\fi
	\begin{tabular}{|c|l|c|}
		\hline
		\textbf{Hypothesis} & \textbf{Summary}                                                    & \textbf{Outcome}    \\
		\hline
		H1                  & Reduces sources and execution time for data-model selective queries & Supported           \\
		H2                  & No effect on non-data-model selective queries                       & Partially supported \\
		H3                  & Negligible query-shape subsumption overhead                         & Supported           \\
		H4                  & Stricter shape constraints reduce \ac{HTTP} requests more           & Rejected            \\
		H5                  & More complete shape indexes give faster execution                   & Rejected            \\
		H6                  & Gains hold under partial shape-index coverage                       & Supported           \\
		H7                  & Linear \ac{HTTP}-request / execution-time relationship              & Partially supported \\
		\hline
	\end{tabular}
	\caption{\rone{Summary of the hypotheses and whether the evaluation supports them.}}
