...
\begin{figure}
    ...
    % General caption
    \caption{
	\rt{This caption needs rephrasing.}
	\rt{The ratio is not explained}
	\rt{Why is type indecx+ldp not in the right figure?}
	\rt{Unclear what No performance change means in the legend of the right figure.}
...
An analysis of statistical significance is provided in the supplementary material~\hyperref[sf:supplementalMaterial]{\footnotemark[\getrefnumber{sf:supplementalMaterial}]}.
...
The poor performance is due to the fact that other approaches rely solely on the reachability \texttt{Cmatch}~\cite{hartig2016walking} to achieve completeness, without leveraging the structural properties of the datasets.\rt{This is incorrect? They also rely on LDP and the type index?}
...
The result tables are available in the supplementary material~\hyperref[sf:supplementalMaterial]{\footnotemark[\getrefnumber{sf:supplementalMaterial}]}.
...
\rt{The formatting of table 1 doesn't look very nice yet.}
...
\begin{figure}
    ...
    \caption{
	\rt{I recommend using absolute times on the Y axis. The ratio is never explained, so it's unclear how this ratio is constructed, and how to understand this figure. Is higher better or worse? And are things linear? This also applies to other figures.}
