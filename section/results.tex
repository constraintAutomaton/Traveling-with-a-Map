...
\begin{figure}
    \includesvg[width=\linewidth]{analysis/artefact/variation_approach/d1_violon_plots}
    \caption{
        Distribution of query execution times (sample size: 50) for queries derived from the D1 template. 
...
In our context, a query that requests a single data model is classified as data-model-selective, since in our benchmark the subwebs contain at most three distinct data model objects.
From the analysis of the queries, those derived from templates D1, D4, S1, and S3 (!!!with the new data we have results!!) are identified as data-model-selective and, as expected, exhibit a reduction in both the number of HTTP requests and the query execution time.
Figure~\ref{fig:violonD1} shows the distribution of query execution times for each instance of the D1 query template.
