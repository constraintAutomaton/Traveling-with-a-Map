...
\rt{This section needs to be split up into subsections.}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\linewidth]{analysis/artefact/variation_approach/reduction_query_execution_time}\rt{Why is the Y axis a ratio and not just absolute exec time? The figure is hard to interpret atm.}
    \caption{
        ...
        The shape index can also answer queries from the S7V3 \rt{What is S7V3?}, which the plot cannot capture.\rt{Why?}
    }
    \label{fig:compApproach}
\end{figure}


Figure~\ref{fig:compApproach} shows that the shape index approach for all query templates \rt{First time you mention query template?} except S4 performs better or comparably to the state-of-the-art (type index with LDP approach).
...
However, those queries were already fast, with the state-of-the-art approach being executed in approximately 0.30\% of the timeout \rt{A bit weird to refer to timeout like this. Just say max exec time instead?}.
...
However, with this current experiment and data, it is difficult to give a conclusive answer.\rt{Reviewers will shoot the paper down because of this. Either we need to find a conclusive answer, or we should omit (part of this) experiment.} \rt{Related to RubenE's article at QuWeDa?}
...
In this analysis, we examine the impact of reducing the shape index information in the network \rt{Alternative formulation: we simulate now all pods exposing shape indexes} and compare the results with a network where each dataset has a complete shape index with detailed shapes.
...
\rt{Important: accept or reject your hypotheses!}
