...
We consider that a shape $S$ is sumbsumed by a query $Q$ denoted as $S \sqsubseteq_{qs} Q$, 
if for any KG $G_s$ respecting the shape there is a graph star pattern in $Q$ that can find a solution.
For a GSP to sumbsumed $S$, we need to consider $Q$ to have the two parts $Q = Q_{\text{body}} \bowtie Q_{\text{unions}}$.
...
\begin{algorithm}[h]
   \caption{Check if a GSP sumbsums a $Q_s$ ($sumbsums_{\text{graph star}}$)}\label{alg:containmentTree}
   \begin{algorithmic}
      ...
      \FORALL{$tp \in Q_{star}$} % O(n_star)
         \IF{\NOT $match(tp, Q_{s\text{body}})$}
            ...
            \FORALL{$q_{us} \in Q_{s\text{unions}}$}
               \IF{$sumbsums_{\text{graph star}}(Q_{star}, Q_{starT_i}, q_{us}, Eval_{star})$}
         ...
         \ELSE
            ...
            \IF{$O(tp) \in  S_{star}(Q_{starT_i})$}
               \IF{\NOT $sumbsums_{\text{graph star}}(Q_{star_{O(tp)}} \in Q_{starT_i}, Q_{starT_i}, Q_s, Eval_{star})$}
                  \RETURN \FALSE
               \ENDIF
            \ENDIF
            %
...
We define the function $sumbsums_{\text{graph star}}$ in Algorithm~\ref{alg:containmentTree} to evaluate whether a GSP with a root star pattern $Q_{star_i}$ from $Q_{starT_i}$ sumbsumes $Q_s$. 
