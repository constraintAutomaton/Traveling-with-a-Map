...
The query given to the engine aims to retrieve information about posts from subweb 3, along with all its replies.
...
\rt{I think this figure comes too late. It could even be mentioned in the introduction already, or a short section thereafter.}
\rt{Also, the caption is insufficient to explain this use case. It really needs to be explained in more detail in the text. This is very important to give the reader an intuitive understanding of the approach.}
\begin{figure}
   \centering
...
However, we believe \rt{Believing is for religions. Be precise here.} they require less effort than VoID descriptions~\cite{Boehm2011}, which have already been used for query optimization for federated queries~\cite{Montoya2017}.
...
A DKG exposing a shape index provides a query engine with the opportunity to reduce its search domain by knowing which resources are query-relevant.\rt{This is a great sentence! I would even considering including it in the introduction already!}
...
\rt{The following paragraph should be mentioned near the start of the paper! Minus the formal terms, since those will be explained afterwards. But such a description will motivate to the reader why these new concepts (query-shape subsumption, ...) are needed in the first place.}
...
To determining whether a source \rt{shape instead of source?} is query-relevant a \emph{query-shape subsumption} problem is solved.  
...
We assume that $q_i$ have no union statement.\rt{Emphasize that this is only for the explanation in this paper, but it's not a limitation of the overall approach.}
\iffalse
