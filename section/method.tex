...
The constraint of shapes $e_i$ intuitively be threated as segments of a star pattern query~\cite{delva2023}.
\citeauthor{spapeExpressionConvert} has written an informative document on the transformation of ShEx shapes into
Select SPARQL queries for validation purposes. 
In this paper we propose to formalize a similar approach but in the form of a simpler \texttt{SELECT} SPARQL query.

\begin{prop}
   In a shape expression $e_i$ the $p$ with combination of the $r$ are transform into a triple pattern $tp$ with a cardinality property path.
