...
This problem involves determining whether a GSP in a query can produce at least one result when evaluated over any source conforming to a specific shape, denoted as $S \sqsubseteq_{qs} Q$.
A common approach for validating shapes over an RDF graph is to convert shapes into SPARQL queries~\sepfootnote{sf:recursiveShape}~\cite{labragayo2017validatingdescribinglinkeddata, Corman2019, Prestamo2023, spapeExpressionConvert}.
...
More formally, for a GSP to be subsumed by a shape $S$, we consider the queries $Q$ and $Q_s$, where $Q_s$ is the query derived from $S$, and both are of the form $Q = Q_{\text{body}} \bowtie Q_{\text{unions}}$.
