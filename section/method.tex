...
\begin{figure}
   ...
   \jr{I think this should be a Listing instead of a figure. The size is also too small. The RDF is also wrong, you have "." at the end of each triple, instead of ";" when reusing the same subject. The predicate si:bindByShape I would rename it to si:boundByShape if possible.}}
   \label{fig:shapeIndex}
\end{figure}
...
\begin{definition}[Graph Star Pattern (GSP)]\label{def:starPattern}
   \jr{I would really like to see an exmple of a GSP. Perhaps now that there is more space we can include it?}
