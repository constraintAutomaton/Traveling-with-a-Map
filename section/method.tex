...
\begin{definition}[Graph Star Pattern]\label{def:starPattern}
   ...
   We define a graph star pattern $Q_{starT}$ as the union between a root star pattern $Q_{star_s}$
   ...
   We then define $Q_{starT}$ given a  set of partial graph star patterns $Q_{starPT}$
   \begin{equation}
...
More formally, instead of traversing the whole subweb $D$ associated with a shape index, the engine will traverse a subweb $D^{\prime} \subseteq D$, ignoring the knowable query-irrelevant sources.
...
We define
$IRI_p = \{ \bigcup SI_i(s_j) | B \sqsubseteq_{qs}  s_j = \mathrm{false}, s_j \in \text{dom}(SI_i) \}$.
...
This problem is analogous to the classic query containment problem, which assesses whether a query can be answered from the answer of another query, that we denote a container query, independent to the database or KG~\cite{afariQCE, Spasi2023}.
In our approach to query-shape containment, we aim to determine whether a graph star pattern in a query can be answered by any source that conforms to a specific shape.
...
In our context, the container query adheres to a graph star pattern template structure, where predicates are always IRIs.
...
We consider that a query $Q$ is contained in a shape $S$, denoted as $Q \sqsubseteq_{qs} S$, if a graph star pattern in $Q$ is contained in $Q_s = T(S)$. 
For a graph star pattern to be contained, we need to consider $Q$ to have the two parts $Q = Q_{\text{body}} \bowtie Q_{\text{unions}}$.
...
\begin{algorithm}[h]
   \caption{Check if a graph contains a star pattern ($isContain_{\text{graph star}}$)}\label{alg:containmentTree}
   \begin{algorithmic}
      ...
      \FORALL{$tp \in Q_{star}$} % O(n_star)
         \IF{\NOT $match(tp, Q_{s\text{body}})$}
            ...
            \FORALL{$q_{us} \in Q_{s\text{unions}}$}
               \IF{$isContain_{\text{graph star}}(Q_{star}, Q_{starT_i}, q_{us}, Eval_{star})$}
         ...
         \ELSE
            ...
            \IF{$O(tp) \in  S_{star}(Q_{starT_i})$}
               \IF{\NOT $isContain_{\text{graph star}}(Q_{star_{O(tp)}} \in Q_{starT_i}, Q_{starT_i}, Q_s, Eval_{star})$}
                  \RETURN \FALSE
               \ENDIF
            \ENDIF
            %
...
We define the function $isContain_{\text{graph star}}$ in Algorithm~\ref{alg:containmentTree} to evaluate whether a graph star pattern with a root star pattern $Q_{star_i}$ from $Q_{starT_i}$ is contained in $Q_s$. 
The algorithm also takes a set $Eval_{star}$ to track which partial graph stars have already been evaluated.
...
To solve $Q \sqsubseteq_{qs} S$, we need to consider the number of graph stars from the BGP with their number of segments in the UGP and the number of BGPs in the UGPs.
