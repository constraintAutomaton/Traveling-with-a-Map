...
To determine whether the contents of a resource conforming to a shape is query-relevant, we extend the existing notion of \emph{query subsumption} to a novel form we call \emph{query-shape subsumption}.
...
Query containment asses whether a query can be answered from the answer of another query, independent of the database or KG~\cite{afariQCE}.
...
Thus, we differ from the standard subsumption problem, as the solution mappings of the super query $Q_2$ must be extendable to answer $Q_1$, and the triple patterns of $Q_2$ must be equivalent to or generalizations of those in $Q_1$.
It is a common approach for shape validation over an RDF graph to convert shapes into SPARQL queries~\sepfootnote{sf:recursiveShape}~\cite{labragayo2017validatingdescribinglinkeddata, Corman2019,Prestamo2023, spapeExpressionConvert}.
...
For a GSP to be subsumed by a shape $S$, we consider the queries $Q$ and $Q_s$, where $Q_s$ is the query derived from $S$, and both are of the form $Q = Q_{\text{body}} \bowtie Q_{\text{unions}}$.
...
\iffalse
A graph star pattern $Q_{\text{starG}_i}$ is contained in $Q_s$ if its segment in $Q_{\text{body}}$ is contained in $Q_s$, and if its segment in at least one $q_i$ in each $Q_u$ is contained in $Q_s$. 
If $Q_{\text{starG}_i}$ is not part of any $Q_u$, then $Q_u$ is ignored.
\fi
\iffalse
...
\iffalse
\rt{I would not mention everything after this, and instead at the starG of this paragraph say that for simplicity of this formalization, we focus on just BGPs of triple patterns and unions.}
In our containment problem, we ignore \texttt{GROUP BY} segments.
We make this choice because, in the context of shape queries, \texttt{GROUP BY} is primarily used to set a cardinality, and we are not attempting to identify resources that can fully answer specific segments of a query. 
Instead, our goal is to disregard resources that are irrelevant to the query, given the constraints imposed by the shape index. 
...
We define the function $subsums_{\text{graph star}}$ in Algorithm~\ref{alg:containmentTree} to evaluate whether a GSP with a root star pattern $Q_{star_i}$ from $Q_{starG_i}$ sumbsumes $Q_s$. 
