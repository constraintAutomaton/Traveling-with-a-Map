...
\sepfootnotecontent{sf:recursiveShape}{
    ...
    and SHACL Inverse Paths \url{https://www.w3.org/TR/shacl\#property-path-inverse} to avoid recurse 
...
\subsection{Descentralized environments with structural properties and completeness under prunning regime}\label{sec:slde}

We propose to focus LTQP on completeness of results instead completeness of traversal in the context of descentralized environments.
To do so, we formalize our approach as follows.
...
For example, if a user has posted a comment, and that comment, among other properties, is a reply to another post, our approach encapsulates these relationships within a single conceptual framework.
This concept serves two purposes: defining targets for validation and capturing relationships between the triple patterns of our query and the corresponding entities.
...
\end{definition}
Thus, the target for the shapes in the shape index is to consider every tree star in the KG of the documents, ensuring that no partial tree stars are shared.
...
The concept of composite reachability criteria allows us to ignore certain sources during traversal based on the knowledge acquired along the way.
...
In our context we are trying to determine if a tree star pattern from our query (given no share partial tree star pattern) can be answer by any sources respecting a specific shape.
...
This is because, under the open-world assumption, the shape defines the minimum topology of a graph.
With this transformation to problem become similar to query containment problems.
...
This structure arises because shapes predominantly describe predicate terms and object terms that are isomorphic to the specific graph.
...
Since we assume that the union statement is not nested (perhaps through rewriting), this limits the number of recursive calls.
