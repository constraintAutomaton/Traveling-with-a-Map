...
\begin{figure*}
    ...
    The query engine starts its query at Subweb~3\rboth{, and the coloured documents are those relevant to the query shown on the left: red for posts and green for comments}.
    }
    \label{fig:dkg}
\end{figure*}
...
The complexity of the problem is reduced by the fact that $Q_s$ has a GSP structure where predicates are always \acp{IRI}.
...
\rone{
\paragraph{Example}
The query in Figure~\ref{fig:dkg} has two graph star patterns.
...
If the triple pattern cannot be found in the \ac{BGP}, the algorithm then looks into the UGPs of $Q_s$.
...
\fi
To solve $Q \sqsubseteq_{qs} S$, we need to consider the number of GSP from the \ac{BGP} with their number of segments in the UGP and the number of \acp{BGP} in the UGPs.
