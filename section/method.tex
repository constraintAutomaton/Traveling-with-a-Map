...
$Q_{body}$ is Basic Graph Pattern (BGP) of the query and $Q_{\text{unions}} = \bigcup Q_u$ are the Union Graph Patterns (UGP) where $Q_u = q_0 \cup q_1 \cup q_2 ... \cup q_n$.
A tree star pattern $Q_{star_i}$ is contained in $Q_s$ its segment in $Q_{body}$ is contained in $Q_s$ and at least one segment of the tree star in $q_i$ in each $Q_u$ is contained in $Q_s$.
If $Q_{star_i}$ is not a part of an $Q_u$, then the $Q_u$ is ignored.
...
We ignore these segments because our focus is not on identifying data sources that can fully answer a specific segment of a query.
Instead, we are disregarding data sources that are irrelevant to the query, given the constraints imposed by the shape index.
      ...
      \FORALL{$tp \in Q_{star}$} % O(n_star)
         ...
         \ELSE
            ...
            \IF{$O(tp) \in  S_{star}(Q_{starT_i})$}
               \IF{\NOT $isContain_{T}(Q_{star_{O(tp)}} \in Q_{starT_i}, Q_{starT_i}, Q_s)$}
                  \RETURN \FALSE
               \ENDIF
            \ENDIF
            %
...
We define the function $isContain_{T}$ defined in Algorithm~\ref{alg:containment} to evaluate if a $Q_{star_i}$ is contained in $Q_s$.
The algorithm is recursive and on its first call $Q_star$ must be the root of the $Q_{star_i}$ and $MS$ must be an empty set, 
this set is used not to duplicate containment calculations.
We notice that the complexity of Algorithm~\ref{alg:containmentTree} is $O(n_{tp}^2 \times n_{union}^2)$
where $n_{tp}$ is the number of triple patterns of $Q_{star_i}$ and $n_{union}$ the number of UGP in $Q_s$.
To solve $Q \sqsubseteq_{qs} S$ we need to consider the $n_{starT}$ tree star from the BGP with their $n_{starTu}$ segment in the UGP
and evaluate them using Algorithm~\ref{alg:containmentTree}.
This operation results in a polynomial time complexity of $O(n_{tp}^2 \times n_{union}^2 \times n_{starT} \times n_{starTu})$.
