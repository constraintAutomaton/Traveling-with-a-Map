...
When a shape $s$ is mapped to an $IRI$, then the KG targeted by the mapping $G = \{g | r \mapsto_{g} g \land iri \mapsto_{iri} r \land \forall iri \in D_{SI}\}$ satisfies $s$.
...
Thus, we differ from the standard subsumption problem, as the solutions of the super query $Q_2$ must be extended to answer $Q_1$.
...
\begin{algorithm}[h]
   \caption{Check if a GSP subsums a $Q_s$ ($subsums_{\text{graph star}}$)}\label{alg:containmentTree}
   \begin{algorithmic}
