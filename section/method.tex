...
In this section, we define our approach by first introducing pruning in LTQP, then explaining what is a shape index, and finally describing how the shape index can be used for pruning in LTQP.
...
The query engine has to build a KG $G^{\prime}$ using a reachability $C^{\prime}$ in its internal data store from the  KG $g$ by dereferencing resources $r \in R$.
...
\end{equation}
for any network $R$.
Since $g \in G^{\prime}$ can only be obtained by dereferencing resources $r \in R$, a smaller $G^\prime$ means fewer HTTP requests to answer a query.  
...
To address this, we introduce the \emph{shape index}, a mapping between RDF document sets and RDF data shapes that describe a subweb controlled by a data provider.  
...
We denote a shape index as \emph{complete} when every shape $s_i \in \text{dom}(SI)$ have a close world assumption~\cite{Gayo2018b, Gayo2018} (we also refer to them as closed) and \emph{incomplete} otherwise.
...
RDF data shapes use \emph{targets} to identify the root node for validation.  
In this work, we assume all entities in a KG within a document follow the same schema.  
...
We extend this by linking star patterns through the object term of a following star pattern, forming a tree-like structure.  
