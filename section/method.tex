...
For a graph star pattern to be contained, we need to consider $Q$ to have the two parts $Q = Q_{\text{body}} \cup Q_{\text{unions}}$.
...
\begin{algorithm}[h]
   \caption{Determine if a graph star pattern is contained ($isContain_{\text{graph star}}$)}\label{alg:containmentTree}
   \begin{algorithmic}
      \algsetup{linenosize=\tiny}
      \scriptsize

      \REQUIRE  $Q_{star}$, $Q_{starT_i}$, $Q_s = Q_{s\text{body}} \cup Q_{s\text{unions}}$ and $Eval_{star}$
      \ENSURE \TRUE $ $ or \FALSE $ $ whether the graph star pattern is contained in the shape
