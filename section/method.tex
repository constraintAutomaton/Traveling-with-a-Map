...
We define 
$IRI_d = \{ \bigcup_{iri \in SI_i(s_j)} iri | \exists B \sqsubseteq_{qs}  s_j, s_j \in SI_i \}$ and 
$IRI_p = \{ \bigcup_{iri \in SI_i(s_j)} iri | \nexists B \sqsubseteq_{qs}  s_j, s_j \in SI_i \}$
...
A tree star pattern $Q_{starT_i}$ is contained in $Q_s$ if its segment in $Q_{body}$ is contained in $Q_s$ and at least one segment of the tree star pattern in $q_i$ in each $Q_u$ is contained in $Q_s$.
...
This operation results in a polynomial time complexity of $O(n_o \times n_{tp}^2 \times n_{union}^2 \times n_{starT} \times n_{starTu})$.
