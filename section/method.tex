...
\begin{figure*}
   ...
   \begin{minipage}[t]{0.45\linewidth}
      \centering
      \lstinputlisting[]{code/shape_index.nq}
   \end{minipage}
   %\hspace{0.05\textwidth}
   % Second figure
   \begin{minipage}[t]{0.45\linewidth}
      \centering
      \lstinputlisting[]{code/gps.rq}
   \end{minipage}
...
\rv{the math typesetting is not proper, when writing $IRI$ it means I times R times I, use this instead (see source)
$\mathit{IRI}$
and you can make that easier with a~macro
\newcommand{\mIRI}{\ensuremath{\mathit{IRI}}}
so you just do
$\mIRI$.
Same with some other words.
}
