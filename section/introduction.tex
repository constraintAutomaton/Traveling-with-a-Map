...
\rt{There's a big concept jump here. A sentence may be needed here for a better transition. You could say something like "To achieve decentralization, ... can be used..."}
...
more pessimist conclusion \rt{Maybe repeat that conclusion in a few words} of the work of \citeauthor{hartig2016walking}~\cite{hartig2016walking} in the context of Open Linked Data without structural properties.
...
In this work, we build on a dataset summarization approach for decentralized environments called a \emph{shape index}~\cite{tam2024opportunitiesshapebasedoptimizationlink} \rt{Be careful with this reference due to double-blind review. Either make sure to always reference it as something existing, or anonymize it and consider it a contribution. I think the latter approach is the safest, and probably better since it increases the contribution size.} to enable link pruning within LTQP.  
\rt{Also, at this stage, it is unclear for the reader what your approach will do. I would expect a high-level intuitive explanation of your approach. Could be limited to a single sentence. (based on query, check relevant shapes in index, and prune others)}
...
\rt{I would recommend being more precise on the contributions of your work. For instance, the containment should be mentioned here as well.}
