...
\rt{A problem statement usually comes at the start of a paper :-) I would recommend moving this section. This could come before or after the place you explain your running example / use case.}
...
\rt{I would expect a new experimental setup section here.}
...
\rt{The following sentences form your motivation right here! This should (also) come much earlier in the paper!}
\rt{Solid probably also deserves a description in related work.}
...
The shape indexes contain 13 triples each, while the largest shapes have up to 150 triples.\rt{Emphasize by saying it's therefore very small...}
...
We then compared our shape index approach to state-of-the-art Solid Pod network traversal algorithms: one leveraging the LDP specification and type index~\cite{Taelman2023} \rt{Is the type index ever explain in detail in your paper? If not, make sure to include it. Probably in the Solid relwork section.}, and another using only the LDP specification~\cite{Taelman2023}, in a network where each Solid Pod provides a complete shape index.
