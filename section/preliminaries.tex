...
\begin{definition}[Triple]\label{def:triple}
    ...
    Where $\mathcal{I}$, $\mathcal{B}$, $\mathcal{L}$,  are respectively the set of every possible IRI, blank node, literal and variable.
\end{definition}
...
A subweb is a DKG that is controlled by a data provider, such that they can provide structure that might be beneficial for query engine optimization.