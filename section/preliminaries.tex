...
\end{equation}
taking a shape expression $e$, a boolean $op$ to determine if the shape is open or closed, and a tree star $G_{starT}$.
This function determine if $G_{starT_i}$ satify $e$ by returning $\mathrm{true}$.
...
$oc_f$ evaluates if an object term of a triple in a $G_{starT_i}$ is of an RDF type, validate a shape by creating 
a $G_{starT_i}^{\prime}$ where $G_{star_i} \in G_{starT_i}^{\prime}$ have as a subject term the targeted object term or 
if it satisfy a literal comparison.
$rs^a_b = \{x \in \mathbb{N}_+ | a < x < b \}$ is a range on the number of triples in a tree star that must respect both $pc_p$ and $oc_f$.
$n \in \{\mathrm{true}, \mathrm{false}\}$ indicate that no triple in a $G_{starT_i}$ must respect both $pc_p$ and $oc_f$. 
...
For $G$ to satisfy $S$, $G$ has to be analyzed as the union of all its tree stars such that $G = \bigcup_{g \in G_{starT}} g$ and that the largest possible $g \in G_{starT}$ are formed.
With this perspective on $G$, for each $e \in E$ part of $S$ and each $G_{starT_i}$ the function $Ve$ is evaluated.
If all the constraints are satisfied for every $G_{starT_i}$, then $G$ satisfies $S$ denoted by $G \models S$.
