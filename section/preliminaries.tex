...
We define a \ac{DKG} as a \ac{KG} $G$ materialized in a network of resources $R$.
A resource $r_i \in R$ is mapped to a \ac{KG} $g_i \subseteq G$, which is a set of triples~\cite{w3ConceptsAbstract}.
We denote this mapping $r_i \mapsto_{\mathcal{G}} g_i$.
A resource is mapped and exposed by an \ac{IRI} $iri_i$ denoted by $iri_i \mapsto_{\mathcal{R}} r_i$.
...
A \emph{data-model object} is a well-defined template of sub-\acp{KG}.
