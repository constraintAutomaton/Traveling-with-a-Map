...
\begin{definition}[Star pattern with dependencies]\label{def:starPattern}
    ...
    that having as a subject term, the object of the star pattern $Q_{starDp_{i-1}}$ where $n$ is the dept where no new star pattern can be generated.
    More formally, we define $Q_{starD}$ as
    \begin{multline}\label{eq:starPattern1D}
     Q_{starDp_0} = Q_{star} \\
     Q_{starDp_1} = Q_{starDp_0} \cup \{tp_i \in Q| \forall o_{tp} \in tp_{ Q_{starDp_0}} = s_{tp_1} \land s_{tp_2} = s_{tp_3} ... = s_{tp_{m_1}}\} \\
     Q_{starDp_2} = Q_{starDp_1} \cup \{tp_i \in Q| \forall o_{tp} \in tp_{ Q_{starDp_1}} = s_{tp_1} \land s_{tp_2} = s_{tp_3} ... = s_{tp_{m_2}}\} \\
     \dotsm \\
     Q_{starD} = Q_{starDp_{n-1}} \cup \{tp_i \in Q| \forall o_{tp} \in tp_{ Q_{starDp_{n-1}}} = s_{tp_1} \land s_{tp_2} = s_{tp_3} ... = s_{tp_{m_n}}\}
    \end{multline},
