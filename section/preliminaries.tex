...
\iffalse
\subsection{data-model selective Queries}
A query is more data-model selective than another if the size of the set of IRI of the where-provenance~\cite{buneman2001and} $prov(Q) \rightarrow \mathcal{I}^2$
of a query $Q_i$ is lower than the size of the set of another query $Q_j$.
In the context of that paper we define a query to be data-model selective in the absolute if t
\fi